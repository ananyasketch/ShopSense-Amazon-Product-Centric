...
class AmazonScraper:
    ...
    def __init__(self, productName, pg_no):
...
# MAIN FUNCTION that takes in the usr query
def execCode(query, brand):
    # Intitla page number
    pg_no = 1
    amz_df_arr = []

    # A variable that creates a 100 second buffer between current time and the end.
    t_end = time.time() + 100

    # Until the current sys time isn't less than the end time variable
    while time.time() < t_end:
        try:
            amz_1 = AmazonScraper(query, pg_no)

            amz_df = amz_1.toDataframe()

            amz_df_arr.append(amz_df)
    ...
    df, brand_present_df = finalDf(amz_df,brand)
